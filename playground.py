...
test_dataset = test_dataset.map(lambda x, y: (random_shift(x), y), num_parallel_calls=tf.data.AUTOTUNE)
test_dataset = test_dataset.map(lambda x, y: (add_noise(x), y), num_parallel_calls=tf.data.AUTOTUNE)
test_dataset = test_dataset.map(lambda x, y: (change_pitch(x), y), num_parallel_calls=tf.data.AUTOTUNE)

validation_dataset = tf.data.Dataset.from_tensor_slices((x_validation, y_validation))

validation_dataset = validation_dataset.map(lambda x, y: (random_shift(x), y), num_parallel_calls=tf.data.AUTOTUNE)
validation_dataset = validation_dataset.map(lambda x, y: (add_noise(x), y), num_parallel_calls=tf.data.AUTOTUNE)
validation_dataset = validation_dataset.map(lambda x, y: (change_pitch(x), y), num_parallel_calls=tf.data.AUTOTUNE)


train_dataset.save('./newData/train_datasetAugmented')
test_dataset.save('./newData/test_datasetAugmented')
validation_dataset.save('./newData/validation_datasetAugmented')
